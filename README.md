...
当前为 Alpha 版本，适配 DSH `0.1.7-rc.2`，不支持 `0.1.6`。`0.1.7` 适配尚未在真实 DSH 中联调，已完成的检查和待验证场景见[宿主接入](docs/INTEGRATION.md#验证状态)。
...
`npm pack` 自动构建插件，生成 `dsh-persona-0.1.0-alpha.4.tgz`。在 DSH 侧边栏打开「插件」，点击「添加插件」，输入该文件的绝对路径并安装。路径必须指向运行 DSH 的电脑上的文件。安装完成后点击「立即启用」。
